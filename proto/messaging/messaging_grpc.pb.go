...
func (c *messagingServiceClient) DeleteChatMessages(ctx context.Context, in *DeleteChatMsgsRequest, opts ...grpc.CallOption) (*DeleteChatMsgsResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(DeleteChatMsgsResponse)
	err := c.cc.Invoke(ctx, MessagingService_DeleteChatMessages_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _MessagingService_DeleteChatMessages_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(DeleteChatMsgsRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(MessagingServiceServer).DeleteChatMessages(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: MessagingService_DeleteChatMessages_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(MessagingServiceServer).DeleteChatMessages(ctx, req.(*DeleteChatMsgsRequest))
	}
	return interceptor(ctx, in, info, handler)
}
